...
export default class QRCode extends PureComponent {
  ...
  render() {
    ...
    const qrcodeSize = size*1.5;
    const backgroundSize = size * 2;
    return (
      <Svg ref={getRef} width={qrcodeSize} height={qrcodeSize}>
        <Rect width={backgroundSize} height={backgroundSize} fill={backgroundColor} />
          <Defs>
          ...
          {path && cellSize && (
            <Path d={path} stroke={color} strokeWidth={cellSize} transform={`translate(${((backgroundSize) - (qrcodeSize))/2} ${((backgroundSize) - (qrcodeSize))/2})`}/>
          )}
          {logo && (
